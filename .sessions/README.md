...
The `control/status.md` `updated:` heartbeat is machine-derived — paste it from `date -u +%Y-%m-%dT%H:%M:%SZ` at write time; never hand-type or round it (a future stamp is rejected by `scripts/check-heartbeat.py`).
