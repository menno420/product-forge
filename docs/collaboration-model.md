# product-forge — collaboration model
...
- When a doc and a source file disagree: Committed product state (code, tests, README, release artifacts) wins over docs; a doc that disagrees is fixed in place with a dated marker. Owner rulings live in superbot's question router (Q-0259 r.4 money protocol, Q-0264 pipeline) and are referenced, never restated.
- Staleness review cadence: Every wake re-checks the newest product's README against its actual state (working/alpha/released honesty); inbox-empty wakes polish the roughest edge and flag the manager. Product READMEs that overstate state are bugs.
