...
updated: 2026-08-20T20:04:47Z
phase: phone-controller Slices 21+22 SHIPPED — keyboard (v0.21.0) + foldables (v0.22.0)
...
ownership recorded as candidate). **Slice 22 SHIPPED (PR #53):** foldables — `smallestScreenSize|density`
configChanges hardening (a fold used to RECREATE the activity and drop the
live connection) + per-screen layout memory (sw600 bucket,
rotation-invariant; compact pad on the cover screen, full pad inside,
automatic). Codex 2 rounds: R1 [conceded]×2 (onCreate bucket restore ·
pending flip through the editor), R2 clean. Designed on the Slice-21 card
§ D5. Card: `.sessions/2026-08-20-phone-controller-slice22-foldables.md`.
...
pattern), SlidePadRouter deleted, tree-wide motion-event splitting → each finger
tracked independently, 4+ simultaneous inputs (digitizer-limited ~10). D-pad glide +
diagonals preserved in DpadView; only face-to-face glide dropped (minor). No
descriptor change. v0.16.0 (versionCode 14). Verified pre-push: 59/59 JVM tests, app
vs android.jar (90 classes). Card:
...
while the Gyro… dialog is open — see it before connecting); recenter (dialog button
+ Analog-pad Gyro long-press); sensitivity slider + invert X/Y; Settings → Gyro…
dialog. Default target = right stick (v0.6.0 behaviour preserved). No descriptor
change. v0.14.0 (versionCode 12). Verified pre-push: 59/59 JVM tests, app vs
android.jar (83 classes). Card:
